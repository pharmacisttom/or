...
$log_file = __DIR__ . '/edit_event_log.txt';
file_put_contents($log_file, "POST Data: " . print_r($_POST, true) . "\n", FILE_APPEND);

$required_fields = [
    'id', 'patient_id', 'surgery_type', 'patient_type', 'anesthesia',
...
if ($all_fields_present) {
    ...
    try {
        ...
        if (isset($_POST['delete']) && $_POST['delete'] == 'on') {
            $sql = "DELETE FROM surgery_bookings WHERE id = :id";
            $query = $bdd->prepare($sql);
            $query->bindParam(':id', $id, PDO::PARAM_INT);
        } else {
            $sql = "UPDATE surgery_bookings SET 
                    ...
                    department_or = :department_or, 
                    post_surgery_booking = :post_surgery_booking,
                    investigations = :investigations,
                    consult_department = :consult_department,
                    blood_booking = :blood_booking
                    WHERE id = :id";
            $query = $bdd->prepare($sql);
        ...
        if ($query->execute()) {
            file_put_contents($log_file, "Success: Data updated/deleted for ID=$id\n", FILE_APPEND);
            echo "OK";
        } else {
            $error = "Error executing query: " . implode(", ", $query->errorInfo());
